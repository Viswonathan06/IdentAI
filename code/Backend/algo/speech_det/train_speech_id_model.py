...
class TripletSemiHardLoss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        # y_true is not used as the loss function does not depend on ground truth labels
        embeddings = tf.math.l2_normalize(y_pred, axis=1)
        batch_size = tf.shape(embeddings)[0]

        # Compute pairwise distance matrix
        distance_matrix = tf.reduce_sum(tf.square(tf.subtract(tf.expand_dims(embeddings, 1), tf.expand_dims(embeddings, 0))), 2)

        # Compute semi-hard negative and positive examples
        mask_anchor_positive = tf.eye(batch_size, dtype=tf.bool)
        mask_anchor_negative = tf.logical_not(mask_anchor_positive)

        anchor_positive_dist = tf.boolean_mask(distance_matrix, mask_anchor_positive)
        hardest_positive_dist = tf.reduce_max(anchor_positive_dist, axis=1, keepdims=True)

        anchor_negative_dist = tf.boolean_mask(distance_matrix, mask_anchor_negative)
        semi_hard_negatives = tf.where(
            anchor_negative_dist - tf.expand_dims(hardest_positive_dist, 1) < self.margin,
            anchor_negative_dist,
            tf.ones_like(anchor_negative_dist) * tf.reduce_max(anchor_negative_dist, axis=1, keepdims=True)
        )

        # Compute triplet loss
        loss = tf.reduce_mean(tf.maximum(0.0, hardest_positive_dist - semi_hard_negatives + self.margin))
