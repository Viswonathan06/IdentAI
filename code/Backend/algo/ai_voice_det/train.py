...
def eval_only(
    ...
    original_preds, confidence_scores = ModelTrainer(batch_size=batch_size, epochs=epochs, device=device).eval(
        model=model,
        dataset_test=dataset_test,
        save_dir=save_dir,
        checkpoint=checkpoint,
    ...
    return original_preds, confidence_scores
...
def experiment(
    ...
    **kwargs,
    ...
    if restore and os.path.exists(restore_path):
    ...
    if evaluate_only:
        results, confidence_scores = eval_only(
            real_dir=path_to_file,
            fake_dir=fake_dir,
            amount_to_use=amount_to_use,
            epochs=epochs,
            device=device,
        ...
        results = [tensor.item() for tensor in results[0]]
        confidence_scores = [tensor.item() for tensor in confidence_scores[0]]

        return {'human_detected':results[:len(results)//2], 
        'confidence_scores':confidence_scores[:len(confidence_scores)//2]}
...
def main():
    ...
    try:
        ...
        experiment(
            ...
            model_dir="./algo/ai_voice_det/saved",
            epochs=args.epochs,
